...
class ProfileManager {
    ...
    async loadCurrentUser() {
        try {
            const response = await fetch(`${this.apiUrl}/${this.currentUserId}`, { timeout: 5000 });
            if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        document.getElementById('bookings-table').innerHTML = '<tr><td colspan="8" class="p-4 text-center text-red-500">Error loading bookings. Please try again.</td></tr>';
    }
...
function populateBookingsTable(bookings) {
    ...
    if (!Array.isArray(bookings) || bookings.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="p-4 text-center text-gray-500">No bookings found.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = bookings.map(booking => {
        const categoryName = categoryMap[booking.venueCategory] || booking.venueCategory;
        const formattedDate = booking.date ? new Date(booking.date).toLocaleDateString('en-GB') : '-';
        const formattedBookedDate = booking.createdAt ? new Date(booking.createdAt).toLocaleDateString('en-GB') : new Date().toLocaleDateString('en-GB');
        const statusClass = booking.status === 'booked' ? 'bg-blue-100 text-blue-600' : booking.status === 'completed' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600';
        const statusText = booking.status ? (booking.status.charAt(0).toUpperCase() + booking.status.slice(1)) : 'Unknown';
        const updatedBadge = booking.updated ? '<span class="updated-badge ml-2">Rescheduled</span>' : '';
        ...
        return `
            <tr class="border-t hover:bg-gray-50 transition-colors table-row" 
                ...
                <td class="p-3 sm:p-4 text-gray-900 text-sm sm:text-base">${booking.roomId || '-'}</td>
                <td class="p-3 sm:p-4 text-gray-900 text-sm sm:text-base">${formattedDate}</td>
                <td class="p-3 sm:p-4 text-gray-900 text-sm sm:text-base">${booking.checkInTime || '-'}</td>
                <td class="p-3 sm:p-4 text-gray-900 text-sm sm:text-base">${booking.checkOutTime || '-'}</td>
                ...
                <td class="p-3 sm:p-4 relative">
                    <button class="actions-btn text-black-900 hover:text-black-700 text-base" 
                            data-booking-id="${booking.id}" aria-haspopup="true" aria-expanded="false">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01"></path>
                        </svg>
                    </button>
                    <div class="actions-dropdown" id="dropdown-${booking.id}" aria-hidden="true"></div>
                </td>
            </tr>
        `;
    }).join('');
    ...
    for (let i = 1; i <= totalPages; i++) {
        html += `
            <button onclick="renderTable(${i})"
                class="px-3 py-1 rounded ${i === currentPage ? 'bg-primary-blue text-white' : 'bg-gray-200 text-gray-700'} hover:bg-primary-blue hover:text-white transition">
                ${i}
            </button>
        `;
    }
...
function setupReviewModal() {
    ...
    function updateStarsDisplay() {
        ...
        stars.forEach(s => {
            s.classList.toggle('selected', parseInt(s.dataset.value) <= effectiveRating);
        });
    }
...
async function openReviewModal(booking) {
    ...
    const successMsg = document.getElementById('review-success').querySelector('p');
    const stars = document.querySelectorAll('.star');
    let selectedRating = 0;
...
async function submitReview(rating) {
    ...
    if (rating < 1 || rating > 5) {
        document.getElementById('review-error').querySelector('p').textContent = 'Please select a rating.';
        document.getElementById('review-error').classList.remove('hidden');
        return;
    }
    const comment = document.getElementById('review-comment').value.trim();
    if (!comment) {
        document.getElementById('review-error').querySelector('p').textContent = 'Please enter a comment.';
        document.getElementById('review-error').classList.remove('hidden');
        return;
    }
    try {
        ...
        if (currentBookingForReview.existingReview) {
            response = await fetch(`${API_URL}/reviews/${currentBookingForReview.existingReview.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
        ...
        document.getElementById('review-error').querySelector('p').textContent = error.message;
        document.getElementById('review-error').classList.remove('hidden');
    }
}
    ...
    try {
        ...
        const hasConflict = allBookings.some(booking => {
            ...
            return (
                ...
                timeOverlaps(booking.checkInTime, booking.checkOutTime, newCheckin, newCheckout)
            );
        });
        availabilityDiv.classList.remove('hidden');
        if (hasConflict) {
            availabilityDiv.innerHTML = '<strong>Unavailable:</strong> This time slot is already booked for the selected room.';
            availabilityDiv.className = 'availability-status unavailable';
            document.getElementById('confirm-reschedule').classList.add('hidden');
        } else {
            availabilityDiv.innerHTML = '<strong>Available:</strong> This time slot is free for booking.';
...
async function confirmReschedule() {
    ...
    const newCheckout = document.getElementById('new-checkout').value;
    try {
        const updateResponse = await fetch(`${API_URL}/bookings/${currentBookingForReschedule.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                date: newDate,
...
async function showBookingModal(booking) {
    ...
    const categoryName = categoryMap[booking.venueCategory] || booking.venueCategory;
    const beveragesList = booking.beverages && typeof booking.beverages === 'object'
        ? Object.entries(booking.beverages).map(([item, qty]) => `${item} x${qty}`).join(', ')
        : (Array.isArray(booking.beverages) ? booking.beverages.join(', ') : 'None');
    modalBody.innerHTML = `
        <!-- Header Section -->
        <div class="bg-primary-blue -mx-6 -mt-6 px-6 py-6 mb-6 rounded-t-xl">
            <div class="flex items-center justify-between">
                ...
                <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${booking.status === 'booked' ? 'bg-blue-100 text-blue-800' : booking.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                    <div class="w-2 h-2 rounded-full mr-2 ${booking.status === 'booked' ? 'bg-blue-500' : booking.status === 'completed' ? 'bg-green-500' : 'bg-red-500'}"></div>
            ...
            <div class="bg-white/95 backdrop-blur-lg rounded-xl p-5 shadow-sm">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-gray-900 font-semibold bg-white px-3 py-1 rounded-lg shadow-sm">${booking.roomId || '-'}</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-gray-600 font-medium">Purpose</span>
                        <span class="text-gray-900 font-semibold">${booking.purpose || '-'}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/95 backdrop-blur-lg rounded-xl p-5 shadow-sm">
                ...
                <div class="space-y-4">
                    <div class="bg-white rounded-lg p-3 shadow-sm">
                        ...
                        <div class="text-lg font-bold text-primary-blue">${booking.date ? new Date(booking.date).toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div class="text-center bg-white rounded-lg p-3 shadow-sm">
                            ...
                            <div class="font-bold text-green-600">${booking.checkInTime || '-'}</div>
                        </div>
                        <div class="text-center bg-white rounded-lg p-3 shadow-sm">
                            <div class="text-xs text-gray-500 uppercase tracking-wide mb-1">Check Out</div>
                            <div class="font-bold text-red-600">${booking.checkOutTime || '-'}</div>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="space-y-4">
    <div class="flex items-center space-x-3">
        ...
        <div>
            <div class="font-semibold text-gray-900">${booking.firstName || '-'} ${booking.lastName || ''}</div>
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            <div class="font-semibold text-gray-900 break-all">${booking.email || '-'}</div>
...
<div class="space-y-4">
    <div class="flex items-center space-x-3">
        ...
        <div>
            <div class="font-semibold text-gray-900">${booking.mobileNumber || '-'}</div>
    ...
    <div class="flex items-start space-x-3">
        ...
        <div>
            <div class="font-semibold text-gray-900 break-words">${booking.address || '-'}</div>
...
<div class="flex items-center justify-between bg-white rounded-lg p-4 shadow-sm">
    <div class="flex items-center space-x-3">
        <div class="w-8 h-8 ${booking.beverage === 'yes' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'} rounded-full flex items-center justify-center">
        ...
        <div>
            ...
            ${booking.beverage === 'yes' ? `<div class="text-sm text-gray-600">Items: ${beveragesList}</div>` : ''}
        </div>
    </div>
    <div class="text-right">
        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${booking.beverage === 'yes' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}">
...
<div class="bg-white rounded-lg p-4 shadow-sm border-l-4 border-green-500">
    <div class="flex items-center justify-between">
        ...
        <div class="text-right">
            <div class="text-2xl font-bold text-green-600">₹${booking.totalCost ?? '-'}</div>
        </div>
    </div>
</div>
</div>
